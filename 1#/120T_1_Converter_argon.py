...
# for day in range(days):
    ...
    if sql_values:
        ...
        for i in sql_values:
            print(i)
            # 前一日最后一笔炉号
            sql = "SELECT  v FROM power.collect_steelmaking  WHERE  ts >= '" + Add_strday_str(date_today,-1) + "' and ts <= '" + date_today + "' and eid='bhdp82cc'  ORDER BY ts desc LIMIT 1 ;"
            # print(sql)
            ...
            if day_last_heatNo == i[0]:
                ...
                smeltv1['heatNo'] = int(i[0])
                smeltv1['argon_begin'] = sql_start_time(date_today,i[0],'bhdp82cc')
                print('炉次:',i[0],'argon_begin:',smeltv1['argon_begin'])
                smeltv1['argon_end'] = sql_end_time(date_today,i[0],'bhdp82cc')
                print('炉次:',i[0], 'argon_end:', smeltv1['argon_end'])
                smeltv1['argon_time'] = sql_max_value(Addsec_dateTime_str(smeltv1['argon_begin'], 1),smeltv1['argon_end'],'styfjo0o')
                print('炉次:', i[0], 'argon_time:', smeltv1['argon_time'])
                wirte_sql()
