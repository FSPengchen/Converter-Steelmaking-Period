...
class once_main():
    ...
    def sql_stop_time_value(self,i, eid):
        sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + str(STOP_start_time_list[i]) + "' and ts < '" + str(STOP_end_time_list[i]) + "' AND  eid='" + eid + "'  ORDER BY ts desc LIMIT 1;"
        # print('采集炉号SQL',sql)
        sql_values = self.client.execute(sql)
        # print('采集炉号返回值', sql_values)
        if sql_values:
    ...
    def sql_stop_start_time(self,i, eid):
        sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts > '" + \
              str(start_time_list[i]) + "' and ts < '" + str(start_time_list[i + 1]) + "' AND  eid='" + eid + "' and v = 1  ORDER BY ts asc LIMIT 1;"
        sql_values = self.client.execute(sql)
        if sql_values:
            sql_values = sql_values[0][0]
        else:
    ...
    def sql_gasrecovery_time(self,i, eid1, eid2, eid3):
        ...
        sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + \
              str(start_time_list[i]) + "' and ts <= '" + str(end_time_list[i]) + "' AND  eid='" + eid2 + "' and v = 1  ORDER BY ts desc LIMIT 1;"
        ...
        if sql_values:
            ...
            # 没有二次，判断一次吹炼状态
            sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + \
                  str(start_time_list[i]) + "' and ts <= '" + str(end_time_list[i]) + "' AND  eid='" + eid1 + "' and v = 1  ORDER BY ts desc LIMIT 1;"
    ...
    def cycle_processing(self,start_time, end_time, date_today, i):
        # str格式转换datetime
        temp_start_time_list = datetime.datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
        temp_end_time_list = datetime.datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S")
        ...
        if temp_start_time_list <= temp_date_today:
            ...
            smeltv1['addIron_begin'] = str(self.sql_start_time(i, 'bhpszecq'))
            print('兑铁开始时间', smeltv1['addIron_begin'])
            smeltv1['addIron_end'] = str(self.sql_start_time(i, 'bhpszecq'))
            ...
            smeltv1['lowerOxy_begin'] = str(self.sql_start_time(i, 'ocmzpdjz'))
            print('吹炼开始时间', smeltv1['lowerOxy_begin'])
            smeltv1['raiseOxy_end'] = str(self.sql_start_time(i, 'ocmzpdjz'))
            ...
            smeltv1['tempMeasur_begin'] = str(self.sql_start_time(i, 'hopv0byf'))
            print('测温取样开始时间', smeltv1['tempMeasur_begin'])
            smeltv1['tempMeasur_end'] = str(self.sql_start_time(i, 'hopv0byf'))
            ...
            smeltv1['lowerTwoOxy_begin'] = str(self.sql_start_time(i, 'viydg03i'))
            print('二吹氧开始时间', smeltv1['lowerTwoOxy_begin'])
            smeltv1['raiseTwoOxy_end'] = str(self.sql_start_time(i, 'viydg03i'))
            print('二吹氧结束时间', smeltv1['raiseTwoOxy_end'])

            smeltv1['moltenSteel_begin'] = str(self.sql_start_time(i, '1aapd39u'))
            print('出钢开始时间', smeltv1['moltenSteel_begin'])

            # 煤气回收，查看二吹，有没有，再查一吹。取最后时间，抓起煤气回收。
            converterGasRecoveryTime_min = int(self.sql_set_time_value(str(smeltv1['moltenSteel_begin']), 'htx3ntld'))
            converterGasRecoveryTime_sec = int(self.sql_set_time_value(str(smeltv1['moltenSteel_begin']), 'kuxu3gox'))
            ...
            smeltv1['lowerNitrogen_begin'] = self.sql_start_time(i, 'mbtgs7u7')
            print('溅渣开始时间', smeltv1['lowerNitrogen_begin'])
            smeltv1['raiseNitrogen_end'] = self.sql_start_time(i, 'mbtgs7u7')
            print('溅渣结束时间', smeltv1['raiseNitrogen_end'])

            smeltv1['pourSlag_begin'] = self.sql_start_time(i, '6doi3zgc')
            print('倒渣开始时间', smeltv1['pourSlag_begin'])
            smeltv1['pourSlag_end'] = self.sql_start_time(i, '6doi3zgc')
            ...
            smeltv1['supplyOxy_begin'] = self.sql_start_time(i, '7skuf2xw')
            ...
            smeltv1['splashSlag_out'] = self.sql_end_time(i, 'y6390ebh')
            print('供氮状态结束时间', smeltv1['splashSlag_out'])

            smeltv1['heatNo'] = int(self.sql_end_time_value(i, 'nixa7gat'))
            print('结束时间的炉号', smeltv1['heatNo'])

            smeltv1['heatAge'] = int(self.sql_end_time_value(i, 'xjxnhbxu'))
            print('结束时间的炉龄', smeltv1['heatAge'])

            addScrap_min = int(self.sql_end_time_value(i, 'teuzcy8i'))
            addScrap_sec = int(self.sql_end_time_value(i, 'n3finsvm'))
            smeltv1['addScrap'] = addScrap_min + float(addScrap_sec / 60)
            print('加废钢时间', smeltv1['addScrap'])

            addIron_min = int(self.sql_end_time_value(i, 'tavq8enf'))
            addIron_sec = int(self.sql_end_time_value(i, 'mtyeippr'))
            smeltv1['addIron'] = addIron_min + float(addIron_sec / 60)
            print('兑铁时间', smeltv1['addIron'])

            supplyOxy_min = int(self.sql_end_time_value(i, '6e3blcd2'))
            supplyOxy_sec = int(self.sql_end_time_value(i, 'gkaoes8k'))
            ...
            if float(smeltv1['supplyOxy']) is not None and float(smeltv1['supplyOxy']) >= 6.0:
                ...
                if sql_values:
                    ...
                    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + temp_start_time_supplyOxy + "' and ts <= '" + temp_end_time_supplyOxy + "' AND eid='xqj57jwr' and v >= '" + str(
            ...
            supplyTwoOxy_min = int(self.sql_end_time_value(i, 'ahchkj6k'))
            supplyTwoOxy_sec = int(self.sql_end_time_value(i, 'faxtlr48'))
            smeltv1['supplyTwoOxy'] = supplyTwoOxy_min + float(supplyTwoOxy_sec / 60)
            print('二吹氧时间', smeltv1['supplyTwoOxy'])

            tempMeasure_min = int(self.sql_end_time_value(i, 'l4ckmhq5'))
            tempMeasure_sec = int(self.sql_end_time_value(i, 'jwsp4h5a'))
            ...
            moltenSteel_min = int(self.sql_end_time_value(i, 'okcmufuv'))
            moltenSteel_sec = int(self.sql_end_time_value(i, 'g4kyba2h'))
            smeltv1['moltenSteel'] = moltenSteel_min + float(moltenSteel_sec / 60)
            print('出钢时间', smeltv1['moltenSteel'])

            splashSlag_min = int(self.sql_end_time_value(i, 'bc35qikq'))
            splashSlag_sec = int(self.sql_end_time_value(i, 'w0zbjcho'))
            smeltv1['splashSlag'] = splashSlag_min + float(splashSlag_sec / 60)
            print('溅渣时间', smeltv1['splashSlag'])

            pourSlag_min = int(self.sql_end_time_value(i, 'find6gdv'))
            pourSlag_sec = int(self.sql_end_time_value(i, 'pfk3wf4c'))
            smeltv1['pourSlag'] = pourSlag_min + float(pourSlag_sec / 60)
            print('倒渣时间', smeltv1['pourSlag'])

            controlOxygenTime_min = int(self.sql_end_time_value(i, 'yuu7auc1'))
            controlOxygenTime_sec = int(self.sql_end_time_value(i, 'eeg0lcgk'))
            smeltv1['controlOxygenTime'] = controlOxygenTime_min + float(controlOxygenTime_sec / 60)
            print('供氧时间', smeltv1['controlOxygenTime'])

            controlNitrogenTime_min = int(self.sql_end_time_value(i, 'wnwqhz2x'))
            controlNitrogenTime_sec = int(self.sql_end_time_value(i, 'msldsqcq'))
    ...
    def main(self):
        ...
        for day in range(self.days):
            ...
            # self.date_today = '2023-02-24'  # 设定执行日期
            sub_day = 0 - self.days
            self.date_today = Date_Time_Arithmetic.Add_strday_str(self.date_today, sub_day)  # 循环每次增加一日

            # try:
            # region 设定时间
            # self.date_today = '2023-03-01'  # 设定执行日期
            # date_today = Date_Time_Arithmetic.Add_strday_str(date_today, day)  # 循环每次增加一日
            ...
            sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + self.date_today + "'  AND (eid='xctr1xno') and  v =0 ORDER BY ts  asc LIMIT 1 ;"
            ...
            sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + self.date_today + "'  AND (eid='xctr1xno') and  v =1 ORDER BY ts  asc LIMIT 1 ;"
            ...
            if process_state == 10:  # 获取当日第一炉次时间
                ...
                # 搜索 第一炉开始时间后的所有数据
                sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + str(everday_first_cycle_start_time) + "' and ts < '" + Date_Time_Arithmetic.Addday_dateTime_str(
                    str(everday_first_cycle_start_time), 1)[:10] + " 00:00:00' AND  eid='" + str(
                    'xctr1xno') + "'  ORDER BY ts asc ;"
            ...
            if process_state == 30:
                ...
                for i in range(len(end_time_list)):
                    ...
                    print('炉次结束时间:', str(end_time_list[i]), type(str(end_time_list[i])))
                    ...
                    # converterGasRecoveryTime_min = int(self.sql_gasrecovery_time(i, 'ocmzpdjz', 'viydg03i', 'htx3ntld'))
                    # converterGasRecoveryTime_sec = int(self.sql_gasrecovery_time(i, 'ocmzpdjz', 'viydg03i', 'kuxu3gox'))
                    # smeltv1['converterGasRecoveryTime'] = converterGasRecoveryTime_min + float(
                    #     converterGasRecoveryTime_sec / 60)
                    # print('煤气回收时间', smeltv1['converterGasRecoveryTime'])
                    ...
                    # 冶炼周期累积
                    smeltPeriod_min = int(self.sql_stop_time_plusone_value(i, 'clefmga6'))
                    smeltPeriod_sec = int(self.sql_stop_time_plusone_value(i, 'rbxooe5o'))
                    ...
                    # 停炉时间累积
                    temporary_min = int(self.sql_stop_time_value(i, 'xegzmlj9'))
                    temporary_sec = int(self.sql_stop_time_value(i, 'e2ryg6kl'))
