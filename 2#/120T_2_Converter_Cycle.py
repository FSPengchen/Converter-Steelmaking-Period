...
2023.2.6
工艺变更:
根据现场要求，进行修改120吨2号转炉，煤气回收时间规则
由吹炼最后一笔数据，改为 出钢状态第一笔数据。并且PLC端也进行同时改动。
...
2023.04.01：
工艺变更:修改测温温度判断规则，在副枪与倾倒测温后到优先。

2023.04.04：
工艺变更:修改引用温度数据,副枪优先,增加副枪测温温度。

2023.04.14:
工艺变更:增加压枪时间，具体的规则，供氧状态停止前 2分钟内，最低枪位持续时间。（具体枪位值） 取最低值，反15 ,当有两个最小值时，取后面的值和时间。

2023.04.24：
工艺变更:增加补炉信息。

2023.05.04
工艺变更:装置温度1，吹氩温度，炉次开始完成后 15分钟读取。冶炼周期增加吹氩温度，炉次结束后15分钟获取数据.

2023.05.08
工艺变更:增加压枪时间，具体的规则，供氧状态停止前 2分钟内，最低枪位持续时间。（具体枪位值） 取最低值，反15 ,当有两个最小值时，取"前"面的值和时间。

2023.05.10
工艺变更:供氧状态来，10分钟之后，点吹炼结束（1》0） 时间，前1分钟，找来看枪位最低点，一直持续的时间。

2023.06.05
PLC程序变更:吹氩的时间取最大值的范围缩减。

2023.06.06
终点关氧前1分钟，1.55米（7845）以下停留时间
...
smeltv1 = {
    ...
    'desulfurization_lance_time':"0",
    'TSC_Carbon_Content':"0",
    'repair_converter':"0",
...
def sql_end_time_temperature_after_argon(i, eid):
    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + \
          str(start_time_list[i]) + "' and ts < '" + str(Date_Time_Arithmetic.Addmin_dateTime_str(str(end_time_list[i]),15)) + "' AND  eid='" + eid + "'  ORDER BY ts desc LIMIT 1;"
    # print('采集炉号SQL',sql)
    try:
        sql_values = client.execute(sql)
    except Exception as e:
...
def sql_gasrecovery_time(i, eid1, eid2, eid3):
    ...
    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + \
          start_time_list[i] + "' and ts <= '" + end_time_list[
...
# 递归查看压枪时间
def desulfurization_lance_time(start_time,end_time):
    # # 在这里增加终点关氧前1分钟，1.55米（7845）以下停留时间
    # # 第一次供氧状态=1  大于10分钟 取值，  小于10分钟为0   供氧结束前 1分半的 枪位低于 1550  的有效时间总和,  到供氧结束时间或者 大于1550。   只记录一次时间
    sql = "SELECT ts, v FROM power.collect_steelmaking WHERE ts >= '" + str(start_time) + "' AND ts <= '" + str(end_time) + "' AND eid = '0082c988' AND v = 0 ORDER BY ts desc LIMIT 1"
    print('在供氧状态内是否有第二次吹氧', sql)
    sql_values = client.execute(sql)
    print('获取的值', sql_values)
    if sql_values:
        print(sql_values[0][0],type(sql_values[0][0]))
        desulfurization_lance_time(Date_Time_Arithmetic.Addsec_dateTime_str(str(sql_values[0][0]), 1),end_time)

    else:
        sql = "SELECT ts, v FROM power.collect_steelmaking WHERE ts >= '" + str(
            Date_Time_Arithmetic.Subsec_dateTime_str(str(end_time), 90)) + "' AND ts <= '" + str(end_time) + "' AND eid = 'b5ad8b64' AND v <=1550 ORDER BY ts asc"
...
def cycle_processing(start_time, end_time):
    ...
    if temp_start_time_list <= temp_date_today:
        ...
        smeltv1['addScrap_begin'] = str(sql_start_time(i, '975a2d9a'))
        print('加废钢开始时间', smeltv1['addScrap_begin'])
        smeltv1['addScrap_end'] = str(sql_end_time(i, '975a2d9a'))
        ...
        smeltv1['addIron_begin'] = str(sql_start_time(i, 'f46bc741'))
        print('兑铁开始时间', smeltv1['addIron_begin'])
        smeltv1['addIron_end'] = str(sql_end_time(i, 'f46bc741'))
        ...
        smeltv1['lowerOxy_begin'] = str(sql_start_time(i, 'a8aef6e9'))
        print('吹炼开始时间', smeltv1['lowerOxy_begin'])
        smeltv1['raiseOxy_end'] = str(sql_end_time(i, 'a8aef6e9'))
        ...
        smeltv1['tempMeasur_begin'] = str(sql_start_time(i, 'a6948894'))
        print('测温取样开始时间', smeltv1['tempMeasur_begin'])
        smeltv1['tempMeasur_end'] = str(sql_end_time(i, 'a6948894'))
        ...
        smeltv1['lowerTwoOxy_begin'] = str(sql_start_time(i, 'd814f6f3'))
        print('二吹氧开始时间', smeltv1['lowerTwoOxy_begin'])
        smeltv1['raiseTwoOxy_end'] = str(sql_end_time(i, 'd814f6f3'))
        print('二吹氧结束时间', smeltv1['raiseTwoOxy_end'])

        smeltv1['moltenSteel_begin'] = str(sql_start_time(i, '75e23142'))
        print('出钢开始时间', smeltv1['moltenSteel_begin'])

        # 煤气回收，查看二吹，有没有，再查一吹。取最后时间，抓起煤气回收。
        converterGasRecoveryTime_min = int(sql_set_time_value(str(smeltv1['moltenSteel_begin']), 'ff54d2a6'))
        converterGasRecoveryTime_sec = int(sql_set_time_value(str(smeltv1['moltenSteel_begin']), '4e724ef6'))
        ...
        smeltv1['lowerNitrogen_begin'] = sql_start_time(i, 'daf94a70')
        print('溅渣开始时间', smeltv1['lowerNitrogen_begin'])
        smeltv1['raiseNitrogen_end'] = sql_end_time(i, 'daf94a70')
        print('溅渣结束时间', smeltv1['raiseNitrogen_end'])

        smeltv1['pourSlag_begin'] = sql_start_time(i, '83ae6f89')
        print('倒渣开始时间', smeltv1['pourSlag_begin'])
        smeltv1['pourSlag_end'] = sql_end_time(i, '83ae6f89')
        ...
        smeltv1['supplyOxy_begin'] = sql_start_time(i, '0082c988')
        print('供氧状态开始时间', smeltv1['supplyOxy_begin'])
        smeltv1['supplyOxy_end'] = sql_end_time(i, '0082c988')
        print('供氧状态结束时间', smeltv1['supplyOxy_end'])

        # 2023.05.10 压枪时间

        if smeltv1['supplyOxy_begin'] != None and smeltv1['supplyOxy_begin'] != 'None' and smeltv1['supplyOxy_end'] != None and smeltv1['supplyOxy_end'] != 'None' and (smeltv1['supplyOxy_end'] - smeltv1['supplyOxy_begin']).seconds >= 600.0:  # 办法有供氧状态开始时间
            # 递归读取压枪时间
            desulfurization_lance_time(smeltv1['supplyOxy_begin'], smeltv1['supplyOxy_end'])

            # 供氧状态来，10分钟之后，点吹炼结束（1》0） 时间，前1分钟，找来看枪位最低点，一直持续的时间。 9cbbc927 枪位   0082c988供氧状态
            # 获取供氧状态10分钟之后到供氧结束时间之间的顺序状态值
            # sql = "SELECT ts,v FROM power.collect_steelmaking WHERE ts >= '"+str(Date_Time_Arithmetic.Addmin_dateTime_str(str(smeltv1['supplyOxy_begin']),10))+"' and ts <= '"+str(smeltv1['supplyOxy_end'])+"' AND  eid='0082c988'  ORDER BY ts asc"
            #
            # print('压枪时间的sql',sql)
            # sql_values = client.execute(sql)
            ...
            #
            #     sql = "SELECT MIN(v), MIN(ts) AS start_time, MAX(ts) AS end_time FROM power.collect_steelmaking WHERE ts >= '"+str(Date_Time_Arithmetic.Submin_dateTime_str(str(first_stop_supplyOxy_time),1))+"' AND ts <= '"+str(first_stop_supplyOxy_time)+"' AND eid='9cbbc927' AND v = (SELECT MIN(v) FROM power.collect_steelmaking WHERE ts >= '"+str(Date_Time_Arithmetic.Submin_dateTime_str(str(first_stop_supplyOxy_time),1))+"' AND ts <= '"+str(first_stop_supplyOxy_time)+"' AND eid='9cbbc927')"
            #     print('这条查询语句会返回最小值、最小值开始时间和最小值结束时间三个结果。其中，最小值是通过子查询找到的，而最小值开始时间和结束时间则是在主查询中通过 MIN 和 MAX 聚合函数计算得出的',sql)
            #     sql_values = client.execute(sql)
            #     if sql_values:
            ...
            #         smeltv1['desulfurization_lance_time'] = (sql_values[0][2] - sql_values[0][1]).total_seconds() / 60
            #
            #     else:
        ...
        #
        #     sql = "SELECT min(v) as min_v,ts as last_ts FROM power.collect_steelmaking WHERE eid = '9cbbc927' AND ts > toDateTime('"+str(smeltv1['supplyOxy_end'])+"') - INTERVAL 2 MINUTE and ts < '"+str(smeltv1['supplyOxy_end'])+"' AND v IS NOT NULL GROUP BY ts ORDER BY ts ASC LIMIT 1"
        ...
        #
        #         sql = "SELECT ts FROM power.collect_steelmaking WHERE ts >= '"+str(sql_ts)+"' and ts < '"+str(smeltv1['supplyOxy_end'])+"' AND  eid='9cbbc927' and v <= "+str(float(sql_v)+15)+" ORDER BY ts desc LIMIT 1"
        ...
        smeltv1['splashSlag_begin'] = sql_start_time(i, '5448d76f')
        print('供氮状态开始时间', smeltv1['splashSlag_begin'])
        smeltv1['splashSlag_out'] = sql_end_time(i, '5448d76f')
        print('供氮状态结束时间', smeltv1['splashSlag_out'])
        # e42fe0ce
        smeltv1['heatNo'] = int(sql_end_time_value(i, '360332d1'))  # 360332d1
        print('结束时间的炉号', smeltv1['heatNo'])

        smeltv1['heatAge'] = int(sql_end_time_value(i, '6151241d'))
        print('结束时间的炉龄', smeltv1['heatAge'])

        addScrap_min = int(sql_end_time_value(i, 'ec299bb7'))
        addScrap_sec = int(sql_end_time_value(i, '03eb4301'))
        smeltv1['addScrap'] = addScrap_min + float(addScrap_sec / 60)
        print('加废钢时间', smeltv1['addScrap'])

        addIron_min = int(sql_end_time_value(i, 'b14886aa'))
        addIron_sec = int(sql_end_time_value(i, 'c2d8fa80'))
        smeltv1['addIron'] = addIron_min + float(addIron_sec / 60)
        print('兑铁时间', smeltv1['addIron'])

        supplyOxy_min = int(sql_end_time_value(i, 'e11b1c10'))
        supplyOxy_sec = int(sql_end_time_value(i, 'ccb0f3f5'))
        ...
        if float(smeltv1['supplyOxy']) is not None and float(smeltv1['supplyOxy']) >= 6.0:
            ...
            try:
                ...
                sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '"+temp_start_time_supplyOxy+"' and ts <= '"+temp_end_time_supplyOxy+"' AND  eid='0082c988' and v = 0  ORDER BY ts desc LIMIT 1;"
                sql_values = client.execute(sql)
                if sql_values:
                    print('在吹炼的1-6分钟内，查看供氧状态是否有为0,有停止供氧')
                    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '"+temp_start_time_supplyOxy+"' and ts <= '"+temp_end_time_supplyOxy+"' AND eid='6d9c9514' and v >= '"+str(set_convert_angle) + "'   ORDER BY ts asc LIMIT 1;"
                    sql_values = client.execute(sql)
                    if sql_values:
        ...
        supplyTwoOxy_min = int(sql_end_time_value(i, '55f52029'))
        supplyTwoOxy_sec = int(sql_end_time_value(i, 'dcda1666'))
        smeltv1['supplyTwoOxy'] = supplyTwoOxy_min + float(supplyTwoOxy_sec / 60)
        print('二吹氧时间', smeltv1['supplyTwoOxy'])

        tempMeasure_min = int(sql_end_time_value(i, '7f4f89fa'))
        tempMeasure_sec = int(sql_end_time_value(i, 'e25967ed'))
        smeltv1['tempMeasure'] = tempMeasure_min + float(tempMeasure_sec / 60)
        print('测温时间', smeltv1['tempMeasure'])

        beforSteelTemp1 = float(sql_end_time_value(i, 'af2d569c'))
        beforSteelTemp2 = float(sql_end_time_temperature_after_argon(i, 'cf0d672c'))
        ...
        moltenSteel_min = int(sql_end_time_value(i, 'd4415fa9'))
        moltenSteel_sec = int(sql_end_time_value(i, 'c7e089dd'))
        smeltv1['moltenSteel'] = moltenSteel_min + float(moltenSteel_sec / 60)
        print('出钢时间', smeltv1['moltenSteel'])

        splashSlag_min = int(sql_end_time_value(i, '474028c6'))
        splashSlag_sec = int(sql_end_time_value(i, '7d658c82'))
        smeltv1['splashSlag'] = splashSlag_min + float(splashSlag_sec / 60)
        print('溅渣时间', smeltv1['splashSlag'])

        pourSlag_min = int(sql_end_time_value(i, '1ddce1a6'))
        pourSlag_sec = int(sql_end_time_value(i, '320b7ebf'))
        smeltv1['pourSlag'] = pourSlag_min + float(pourSlag_sec / 60)
        print('倒渣时间', smeltv1['pourSlag'])

        controlOxygenTime_min = int(sql_end_time_value(i, '01c531f1'))
        controlOxygenTime_sec = int(sql_end_time_value(i, 'dec4e4fd'))
        smeltv1['controlOxygenTime'] = controlOxygenTime_min + float(controlOxygenTime_sec / 60)
        print('供氧时间', smeltv1['controlOxygenTime'])

        controlNitrogenTime_min = int(sql_end_time_value(i, '4b6d5fce'))
        controlNitrogenTime_sec = int(sql_end_time_value(i, '915a794d'))
...
while flag:
    ...
    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + date_today + "'  AND (eid='129756f8') and  v =0 ORDER BY ts  asc LIMIT 1 ;"
    ...
    sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + date_today + "'  AND (eid='129756f8') and  v =1 ORDER BY ts  asc LIMIT 1 ;"
    ...
    if everday_earliest_cycle_STOP_time and everday_earliest_cycle_RUN_time:
        ...
        if everday_earliest_cycle_RUN_time[0][0] > everday_earliest_cycle_STOP_time[0][0]:
            ...
            sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + str(everday_earliest_cycle_STOP_time) + "'  AND (eid='129756f8') and  v =1 ORDER BY ts  asc LIMIT 1 ;"
    ...
    if process_state == 10:  # 获取当日第一炉次时间
        ...
        sql = "SELECT ts ,v FROM power.collect_steelmaking WHERE ts >= '" + str(everday_first_cycle_start_time) + "' and ts < '" + str(Date_Time_Arithmetic.Addday_dateTime_str(
            str(everday_first_cycle_start_time), 1)[:10]) + " 00:00:00' AND  eid='129756f8'  ORDER BY ts asc ;"
        ...
        if sql_values:  # 判断有数据获取
            ...
            for i in range(0, len(sql_values)):
                # print(i, sql_values[i],start_state_temp)
                # 判断结果大于0的值和上一周的值是否一致,过滤功能
                # print('查看',sql_values[i][1],start_state_temp,sql_values[i],i)
                if sql_values[i][1] > start_state_temp:
                    # 将大于0的值 赋值这一周期,即0 -> 1
                    start_state_temp = 1  # 2023.04.01 修改
                    # 保存下来 0 -> 1 的时间存入数组
    ...
    if process_state == 20:
        ...
        # 炉次结束时间
        # 修改记录 2023.04.01 修改 由 start_time_list  改为 STOP_start_time_list
        ...
        if len(start_time_list) == len(end_time_list):
            ...
            print('异常~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            # 2023.04.01  增加 process_state = 30
            process_state = 30
    # 处理炉次信息
    if process_state == 30:
        print('第三阶段:处理炉次信息')
        ...
        for i in range(len(end_time_list)):
            ...
            # converterGasRecoveryTime_min = int(sql_gasrecovery_time(i, 'a8aef6e9', 'd814f6f3', 'ff54d2a6'))
            # converterGasRecoveryTime_sec = int(sql_gasrecovery_time(i, 'a8aef6e9', 'd814f6f3', '4e724ef6'))
            #
            #
            #
            # smeltv1['converterGasRecoveryTime'] = converterGasRecoveryTime_min + float(
            ...
            # 冶炼周期累积
            smeltPeriod_min = int(sql_stop_time_plusone_value(i, 'e150a8de'))
            smeltPeriod_sec = int(sql_stop_time_plusone_value(i, '9ca47ee5'))
            ...
            # 停炉时间累积
            temporary_min = int(sql_stop_time_value(i, 'e02914b1'))
            temporary_sec = int(sql_stop_time_value(i, 'bcff27ea'))
